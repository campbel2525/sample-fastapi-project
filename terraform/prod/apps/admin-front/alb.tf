...
resource "aws_lb_target_group" "alb_app_target_group" {
  name                 = "${var.project_name}-${var.environment}-alb-admin-front-tg"
  target_type          = "ip"
...
resource "aws_lb_listener_rule" "redirect_http_to_https" {
  ...
  tags = {
    Name    = "${var.project_name}-${var.environment}-admin-front-http"
    Project = var.project_name
    Env     = var.environment
  }
}
...
resource "aws_lb_listener_rule" "for_ecs_app" {
  ...
  tags = {
    Name    = "${var.project_name}-${var.environment}-admin-front-https"
    Project = var.project_name
    Env     = var.environment
  }
}
