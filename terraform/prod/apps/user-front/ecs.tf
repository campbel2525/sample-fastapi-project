...
resource "aws_ecs_task_definition" "ecs_app" {
  family                   = "${var.project_name}-${var.environment}-user-front-app" # タスク定義名のプレフィックス
  cpu                      = "256"
  memory                   = "512"
  network_mode             = "awsvpc"
  ...
  container_definitions    = <<EOF
    [
      {
        "name": "${var.project_name}-${var.environment}-user-front-app",
        ...
        "logConfiguration": {
          ...
          "options": {
            ...
            "awslogs-group": "/aws/ecs/${var.project_name}/${var.environment}/user-front-app",
            "awslogs-stream-prefix": "${var.project_name}-${var.environment}-user-front-app"
          }
        },
        "portMappings": [
          {
        ...
        "secrets": [
          {
            "name": "APP_ENV_VALUES",
            "valueFrom": "arn:aws:ssm:${var.aws_region}:${var.aws_account_id}:parameter/ecs/${var.project_name}/${var.environment}/user-front/.env"
          }
        ]
      }
    ]
...
resource "aws_ecs_service" "ecs_app" {
  ...
  load_balancer {
    ...
    container_name   = "${var.project_name}-${var.environment}-user-front-app"
    container_port   = 8000
  }
...
resource "aws_cloudwatch_log_group" "app" {
  name              = "/aws/ecs/${var.project_name}/${var.environment}/user-front-app"
