...
resource "aws_ecs_task_definition" "ecs_app" {
  family                   = "${var.project_name}-${var.environment}-user-api" # タスク定義名のプレフィックス
  cpu                      = "256"
  memory                   = "512"
  network_mode             = "awsvpc"
  ...
  container_definitions    = <<EOF
    [
      {
        "name": "${var.project_name}-${var.environment}-user-api-app",
        ...
        "logConfiguration": {
          ...
          "options": {
            ...
            "awslogs-group": "/aws/ecs/${var.project_name}/${var.environment}/user-api-app",
            "awslogs-stream-prefix": "${var.project_name}-${var.environment}-user-api-app"
          }
        },
        "portMappings": [
          {
        ...
        "secrets": [
          {
            "name": "APP_ENV_VALUES",
            "valueFrom": "arn:aws:ssm:${var.aws_region}:${var.aws_account_id}:parameter/ecs/${var.project_name}/${var.environment}/user-api/.env"
          }
        ]
      }
    ]
...
resource "aws_cloudwatch_log_group" "app" {
  name              = "/aws/ecs/${var.project_name}/${var.environment}/user-api-app"
