...
resource "aws_db_parameter_group" "mysql_standalone_parametergroup" {
  name   = "${var.project_name}-${var.environment}-mysql-standalone-parametergroup"
  family = "mysql8.0"
...
resource "aws_db_option_group" "mysql_standalone_optiongroup" {
  name                 = "${var.project_name}-${var.environment}-mysql-standalone-optiongroup"
  engine_name          = "mysql"
...
resource "aws_db_subnet_group" "mysql_standalone_subnetgroup" {
  name = "${var.project_name}-${var.environment}-mysql-standalone-subnetgroup"
  subnet_ids = [
...
resource "aws_db_instance" "mysql_standalone" {
  ...
  # DB インスタンス識別子情報
  identifier = "${var.project_name}-${var.environment}-mysql-standalone"
  ...
  tags = {
    Name    = "${var.project_name}-${var.environment}-mysql-standalone"
    Project = var.project_name
    Env     = var.environment
  }
}
